...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    vol.Optional(CONF_TYPE, default=SENSOR_TYPES[ATTR_STDEV]):
...
def calc_stdev(sensor_values, round_digits):
    ...
    if count == 0 :
        return None
    return round(statistics.stdev(vals), round_digits)
...
class MinMaxSensor(Entity):
    ...
    async def async_update(self):
        ...
        sensor_values = [self.states[k] for k in self._entity_ids
                         if k in self.states]
        self.stdev = calc_stdev(sensor_values, self._round_digits)
